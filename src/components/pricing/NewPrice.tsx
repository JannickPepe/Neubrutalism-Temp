...
type PricingPlan = "M" | "A";

export default function NewPricing() {
    const [selected, setSelected] = useState<PricingPlan>("M");

    return (
        <section className="w-full text-black  px-4 lg:px-8 py-12 lg:py-16 relative overflow-hidden">
...
interface HeadingProps {
    ...
    setSelected: React.Dispatch<React.SetStateAction<PricingPlan>>;
}
...
const BackgroundShift: React.FC = () => (
    <motion.span layoutId="bg-shift" className="absolute inset-0 bg-black rounded-lg -z-10" />
);
...
const CTAArrow: React.FC = () => (
    <div className="absolute -right-[100px] top-2 sm:top-0">
        ...
        <span className="block text-xs w-fit bg-[#FF6154] text-white shadow px-1.5 py-0.5 rounded -mt-1 ml-8 -rotate-2 font-light italic">
            Save $$$
        </span>
    </div>
);
...
const PriceCards: React.FC<PriceCardsProps> = ({ selected }) => (
        ...
        <div className="w-full bg-white p-6 border-[1px] border-slate-300 rounded-xl">
            ...
            <div className="overflow-hidden mb-8">
                ...
                {selected === "M" ? (
                    ...
                    className="text-6xl font-bold text-[#FF6154]"
                    >
                    <span>$49</span>
            ...
            <motion.button
                whileHover={{ scale: 1.015 }}
                whileTap={{ scale: 0.985 }}
                className="w-full py-4 mt-8 font-semibold bg-[#d43122] text-white rounded-lg uppercase"
            >
...
const BottomRightCircle: React.FC = () => {
    return (
        <motion.div
            initial={{ rotate: "0deg" }}
            animate={{ rotate: "-360deg" }}
